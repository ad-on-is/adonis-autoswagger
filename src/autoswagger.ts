...
export class AutoSwagger {
  ...
  private standardTypes = ['string', 'number', 'integer', 'datetime', 'boolean']
  ...
  private getSchemaExampleBasedOnAnnotation(
    ...
    for (const [key, value] of Object.entries(properties)) {
      ...
      if (rel !== '') {
        ...
        if (typeof props[key.replace('_id', '')] !== 'undefined') continue
  ...
  */
  ...
  private async getSchemas() {
    ...
    schemas = { ...schemas, ...(await this.getInterfaces()), ...(await this.getModels()) }
  ...
  private async getInterfaces() {
    let interfaces = {}
    const files = await this.getFiles(path.join(this.options.path, '/Interfaces'), [])
    const readFile = util.promisify(fs.readFile)
    for (let file of files) {
      const data = await readFile(file, 'utf8')
      file = file.replace('.ts', '')
  ...
  private parseInterfaces(data) {
    ...
    lines.forEach((line, index) => {
      line = line.trim()
      if (line.startsWith('export') && !line.startsWith('export default')) return
